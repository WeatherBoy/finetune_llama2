...
def main(model_to_run):
    ...
    if model_to_run == "new_model":
        ...
        new_model_response = generate_response(Config.HF_HUB_MODEL_NAME, new_tokenizer, prompt)
        print("Response from new model:")
        print(new_model_response)
    else:
        llama_model_name = Config.MODEL_NAME
        llama_tokenizer = AutoTokenizer.from_pretrained(llama_model_name)
        llama_model_response = generate_response(llama_model_name, llama_tokenizer, prompt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_to_run", type=str, help='Which model to run: "new_model" or "llama2"')
